...
- **Figures**: a folder that consists the following subfolders:
  ...
  - *From_Pairwise_to_HIgher-order_Clustering*: it contains the following subfolders:
    ...
    - binarized_adjacency_matrices: a folder that contains of binarized adjacency matrices of $\mathcal{G}$, $\mathcal{G}^{(0)}_{II}$, $\mathcal{G}^{(0)}_{TC}$, $\mathcal{G}^{(4)}_{II}$, $\mathcal{G}^{(4)}_{TC}$ (for different values of density $D \in \{0\%,5\%,15\%,20\%\}$), with rows and columns ordered based on the clustering outcomes;
